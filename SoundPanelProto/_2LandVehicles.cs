...
{
    public partial class _2LandVehicles : Form
    {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM tbl_KINDERLANDVEHICLES WHERE SoundID = @SoundId AND SoundData IS NOT NULL", connection))
            {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("SELECT SoundName, SoundData FROM tbl_KINDERLANDVEHICLES WHERE SoundID = @SoundId", connection))
            {
                cmd.Parameters.AddWithValue("@SoundId", soundId);
        ...
        private bool HasImageData(SqlConnection connection, int dataId)
        {
            ...
            using (SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM tbl_KINDERLANDVEHICLES WHERE SoundID = @DataId AND ImageData IS NOT NULL", connection))
            {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("SELECT ImageData FROM tbl_KINDERLANDVEHICLES WHERE SoundID = @DataId", connection))
            {
                cmd.Parameters.AddWithValue("@DataId", dataId);
        ...
        private void UpdateDataInDatabase(byte[] dataBytes, int rowId, string columnName)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string sqlQuery = $"UPDATE tbl_KINDERLANDVEHICLES SET {columnName} = @{columnName} WHERE SoundID = @ID";
        ...
        private byte[] RetrieveSoundDataFromDatabase(int soundId)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string sqlQuery = "SELECT SoundData FROM tbl_KINDERLANDVEHICLES WHERE SoundID = @ID";
        ...
        private string GetFilePathFromLoadableSounds(int soundID)
        {
            ...
            {
                connection.Open();
                SqlCommand cmd = new SqlCommand("SELECT SoundData FROM tbl_KINDERLANDVEHICLES WHERE SoundID = @SoundID", connection);
                cmd.Parameters.AddWithValue("@SoundID", soundID);
                ...
                if (!string.IsNullOrEmpty(textBox1.Text) && imageData != null)
                {
                    using (OpenFileDialog openFileDialog = new OpenFileDialog() { Filter = "Sound Files|*.wav;*.mp3|All Files|*.*" })
                    {
                        if (openFileDialog.ShowDialog() == DialogResult.OK)
                        {
                            ...
                            using (SqlConnection connection = new SqlConnection(connectionString))
                            using (SqlCommand cmd = new SqlCommand("UPDATE tbl_KINDERLANDVEHICLES SET SoundData = @SoundData WHERE SoundID = @ID", connection))
                            {
                                connection.Open();
        ...
        private void InsertBtn_Click(object sender, EventArgs e)
        {
            if (int.TryParse(label3.Text, out int soundID))
            {
                ...
                else
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    using (SqlCommand cmd = new SqlCommand("UPDATE tbl_KINDERLANDVEHICLES SET ImageData = @ImageData WHERE SoundID = @ID", connection))
                    {
                        try
                        {
                            connection.Open();
                    ...
                    if (updateSuccessful && label3.Text == "1")
                    {
                        using (SqlConnection connection = new SqlConnection(connectionString))
                        {
                            try
                            {
                                ...
                                using (SqlCommand cmd = new SqlCommand("UPDATE tbl_KINDERLANDVEHICLES SET SoundName = @Name WHERE SoundID = @ID", connection))
                                {
                    ...
                    else if (label3.Text == "2")
                    {
                        using (SqlConnection connection = new SqlConnection(connectionString))
                        {
                            try
                            {
                                ...
                                // Update the Name in tbl_KINDERLANDVEHICLES
                                using (SqlCommand cmd = new SqlCommand("UPDATE tbl_KINDERLANDVEHICLES SET SoundName = @Name WHERE SoundID = @ID", connection))
                                {
                                    cmd.Parameters.AddWithValue("@Name", soundName);
        ...
        private void pictureBox2_Click(object sender, EventArgs e)
        {
            ...
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                using (SqlCommand cmd = new SqlCommand("UPDATE tbl_KINDERLANDVEHICLES SET SoundData = NULL, ImageData = NULL WHERE SoundID = @ID", connection))
                {
                    try
                    {
                        connection.Open();
